...
class Client
{
    ...
    function addInBDD()
    {
      $db = connect();
			if($db)
      {
				$sql = 'INSERT INTO client(name,age) VALUES("'.$this->_name.'",  "'.$this->_age.'")';
				$result = mysqli_query($db,$sql);
				if($result)
				{
          print "Le client a été ajouté en base de données.\n";
          header('Location: ../view/showClients.php');
        }
      }
    }
    ...
    function updateInBDD()
    {
      ...
      if($db)
      {
        $sql = 'UPDATE client SET name="'.$this->_name.'", age="'.$this->_age.'" WHERE id='.$this->_id.'';
        $result = mysqli_query($db,$sql);
        if($result)
        {
