    ...
    if($result)
    {
      while ($row = mysqli_fetch_array($result)) {
        ...
        if(strcmp($row["idAssurance"], -1) == 0)
        {
          ...
          $res .= '<td><a class="btn-primary btn-sm" href="../view/handleInsuranceToCar.php?id='.$row['idVoiture'].'">Ajouter assurance</a></td>';
        }
        else {
          //Afficher l'assurance choisie
          $res .= '<td>'.findAssurance($row['idAssurance']);
          $res .= '  <a class="btn-success btn-sm" href="../view/handleInsuranceToCar.php?id='.$row['idVoiture'].'">Modifier assurance</a></td>';
        }
        $res .= '</tr>';
      }
    }
